...
namespace bsc {
    ...
    private:
        ...
        KeyType getKey(const char* s) const { return getKey(std::string(s)); }
    ...
    public:
        ...
        Entry<ContextValueType> setDirect(std::shared_ptr<RealValueType> valuePtr) {
            ...
            if constexpr (std::is_base_of_v<ContextValueType, RealValueType>) {
                ...
                throw InvalidContextException("Cannot cast "s + typeid(RealValueType).name() + "to " + typeid(ContextValueType).name());
            }
        }
