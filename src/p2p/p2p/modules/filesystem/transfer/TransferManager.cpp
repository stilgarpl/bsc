...
namespace bsc {
    ...
    const std::shared_ptr<std::istream>& TransferManager::RemoteTransferDescriptor::getInputStream() const { return inputStream; }
    ...
    TransferSize TransferManager::RemoteTransferDescriptor::getSize() const { return size; }

    void TransferManager::RemoteTransferDescriptor::setSize(TransferSize size) { RemoteTransferDescriptor::size = size; }
    ...
    void TransferManager::saveDataChunk(const std::shared_ptr<std::ostream>& outputStream,
                                        const TransferSize& begin,
    ...
    TransferManager::LocalTransferDescriptorPtr TransferManager::initiateTransfer(const NodeIdType& nodeId,
                                                                                  const ResourceIdentificatorPtr& source,
    ...
    TransferManager::initiateTransfer(ResourceIdentificatorPtr source, ResourceIdentificatorPtr destination, bool start) {
        ...
        //@todo this broadcast shouldn't happen at the moment of ordering the request, but at the time of actual run. but it's not high
        // priority, most of the time it won't matter broadcast who has
        TransferQuery::Request::Ptr request = TransferQuery::Request::getNew();
        request->setResourceIdentificator(source);
        //@todo this will block... maybe do it in async way? it would be best to do all the broadcast stuff at the start of the transfer
        // thread
    ...
    TransferManager::LocalTransferDescriptor::DefinitionPtr TransferManager::LocalTransferDescriptor::makeDefinition() {

        static auto ptr = std::make_shared<Definition>([]() {
            Definition definition;
            definition.addState(TransferState::NOT_STARTED,
                                TransferState::PREPARED,
                                TransferState::ATTRIBUTES_ACCQUIRED,
                                TransferState::DOWNLOADING,
                                TransferState::STARTED,
                                TransferState::ERROR,
                                TransferState::FINISHED);
            definition.addLink(TransferState::NOT_STARTED, TransferState::PREPARED, TransferState::STARTED, TransferState::ERROR);
            definition.addLink(TransferState::PREPARED, TransferState::STARTED, TransferState::ERROR);
            definition.addLink(TransferState::STARTED, TransferState::ATTRIBUTES_ACCQUIRED, TransferState::ERROR);
            definition.addLink(TransferState::ATTRIBUTES_ACCQUIRED, TransferState::DOWNLOADING, TransferState::ERROR);
            definition.addLink(TransferState::DOWNLOADING, TransferState::FINISHED, TransferState::ERROR);
            definition.addLink(TransferState::ERROR, TransferState::NOT_STARTED);
            return definition;
        }());
        return ptr;
    }

    TransferManager::LocalTransferDescriptor::LocalTransferDescriptor() : LogicStateMachine(makeDefinition(), *this) {
        setState(TransferState::NOT_STARTED);
    }

    void TransferManager::LocalTransferDescriptor::setPayload(const std::function<void(LocalTransferDescriptor&)>& p) { payload = p; }
    ...
    void TransferManager::LocalTransferDescriptor::startThread() {
        ...
        if (thread == nullptr) {
            ...
            thread = std::make_unique<std::thread>(
                    [=, this](LocalTransferDescriptor& descriptor) {
    ...
    const ResourceIdentificatorPtr& TransferManager::LocalTransferDescriptor::getDestination() const { return destination; }
    ...
    const ResourceIdentificatorPtr& TransferManager::LocalTransferDescriptor::getSource() const { return source; }
    ...
    const NodeIdType& TransferManager::LocalTransferDescriptor::getSourceNode() const { return sourceNode; }
    ...
    const std::chrono::time_point<std::chrono::steady_clock>& TransferManager::LocalTransferDescriptor::getStartTime() const {
        return startTime;
    }

    void TransferManager::LocalTransferDescriptor::setStartTime(const std::chrono::time_point<std::chrono::steady_clock>& startTime) {
    ...
    void TransferManager::LocalTransferDescriptor::setEndTime(const std::chrono::time_point<std::chrono::steady_clock>& endTime) {
        LocalTransferDescriptor::endTime = endTime;
    }

    void TransferManager::TransferQueue::queueTransfer(const NodeIdType& nodeId,
                                                       const ResourceIdentificatorPtr& source,
                                                       const ResourceIdentificatorPtr& destination) {
    ...
    TransferManager::TransferQueue::TransferQueue(TransferManager& manager) : LogicStateMachine(makeDefinition(), *this), manager(manager) {
    ...
    void TransferManager::TransferQueue::queueTransfer(ResourceIdentificatorPtr source, ResourceIdentificatorPtr destination) {
        {
    ...
    TransferManager::TransferQueue::DefinitionPtr TransferManager::TransferQueue::makeDefinition() {
        static auto ptr = std::make_shared<Definition>([]() {
            Definition definition;
            definition.addState(TransferState::NOT_STARTED,
                                TransferState::PREPARED,
            ...
            definition.addLink(TransferState::NOT_STARTED, TransferState::PREPARED, TransferState::STARTED, TransferState::ERROR);
            definition.addLink(TransferState::PREPARED, TransferState::STARTED, TransferState::ERROR);
            definition.addLink(TransferState::STARTED,
                               TransferState::ATTRIBUTES_ACCQUIRED,
                               TransferState::DOWNLOADING,
                               TransferState::ERROR);
            definition.addLink(TransferState::ATTRIBUTES_ACCQUIRED, TransferState::DOWNLOADING, TransferState::ERROR);
            definition.addLink(TransferState::DOWNLOADING, TransferState::FINISHED, TransferState::ERROR);
            definition.addLink(TransferState::FINISHED, TransferState::NOT_STARTED, TransferState::ERROR);
            definition.addLink(TransferState::ERROR, TransferState::NOT_STARTED);
            return definition;
        }());
        return ptr;
