        ...
        public:
            ...
            ThisType& fireModuleAction(RetType (ModuleType::*f)(ModArgs... args)) {
                ...
                if constexpr (sizeof...(ModArgs) == 0) {
                    ...
                    fireNewAction(std::bind(f, node.getModule<ModuleType>(), _1));
                } else if constexpr (sizeof...(ModArgs) == 2) {
                    fireNewAction(std::bind(f, node.getModule<ModuleType>(), _1, _2));
                } else if constexpr (sizeof...(ModArgs) == 3) {
                    fireNewAction(std::bind(f, node.getModule<ModuleType>(), _1, _2, _3));
                }
                return *this;
            }

            ModuleLogicChainHelper(const EventHelper<EventType, Args...>& eventHelper, LogicManager& l, LogicObject& lo, INode& node)
                : SpecificLogicChainHelper<ModuleLogicChainHelper, EventType, Args...>(eventHelper, l, lo), node(node) {
            }

            template<typename NewThisType>
            ModuleLogicChainHelper(const EventHelper<EventType, Args...>& eventHelper,
                                   LogicManager& l,
                                   LogicObject& lo,
                                   const NewThisType& self)
                : SpecificLogicChainHelper<ModuleLogicChainHelper, EventType, Args...>(eventHelper, l, lo, self), node(self.node) {
            }

            template<typename NewEventType, typename... NewArgs>
            friend class ModuleLogicChainHelper;
        };

        //    template<typename EventType, typename ... Args>
        //    class StateModuleLogicChainHelper : public ModuleLogicChainHelper<EventType, Args...> {
        //    public:
        //        typename LogicChainHelper<EventType, Args...>::ThisType& fireStateChangeReaction(std::function<void(typename
        //        EventType::Type&)> reaction) {
        //            return LogicChainHelper<EventType, Args...>::fireNewAction([reaction](auto event){reaction(event.getObject());});
        //        }
        //
        //        StateModuleLogicChainHelper(const EventHelper<EventType, Args...> &eventHelper, LogicManager &l, INode &node) :
        //        ModuleLogicChainHelper<EventType, Args...>(eventHelper,l,node){};
        //    };
    protected:
        template<typename EventType, typename... Args>
