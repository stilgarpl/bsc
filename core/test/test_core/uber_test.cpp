...
TEST_CASE("Uber test") {
    ...
    for (unsigned i = 0; i < MAX_TEST_INT; i++) {
        uberList.get<int>().push_back(i);
    }
    for (unsigned i = 0; i < MAX_TEST_FLOAT; i++) {
        uberList.get<float>().push_back(0.5f * i);
    }
    for (unsigned i = 0; i < MAX_TEST_STRING; i++) {
    ...
    }
    {
        ...
        for (unsigned i = 0; i < MAX_TEST_FLOAT; i++, it++) {
            REQUIRE(*it == 0.5f * i);
        }
    }
    {
        auto it = uberList.get<std::string>().begin();
        for (unsigned i = 0; i < MAX_TEST_STRING; i++, it++) {
