...
//                         std::this_thread::sleep_for(3s);
    ...
    TransferManager::LocalTransferDescriptorPtr
    TransferManager::initiateTransfer(ResourceIdentificatorPtr source, ResourceIdentificatorPtr destination,
                                      bool start) {
...
//        LOGGER("response from " + nodeId)
    ...
    const std::chrono::time_point<std::chrono::steady_clock>&
    TransferManager::LocalTransferDescriptor::getEndTime() const {
        return endTime;
    }
    ...
    TransferManager::TransferQueue::TransferQueue(TransferManager& manager) : LogicStateMachine(*this),
                                                                              manager(manager) {
...
//    LOGGER("transfers to start " + std::to_string(transfersToStart));
    ...
    auto TransferManager::TransferQueue::countTransfersNotInState(
            TransferManager::TransferState state) -> decltype(transfers.size()) {
    ...
    auto TransferManager::TransferQueue::countTransfersInState(
            TransferManager::TransferState state) -> decltype(transfers.size()) {
...
//    return result;
    ...
    void
    TransferManager::TransferQueue::queueTransfer(ResourceIdentificatorPtr source,
    ...
    void TransferManager::TransferQueue::waitToFinishAllTransfers() {
        ...
        finishReady.wait(g, [this] {
            return this->getCurrentState() == TransferState::FINISHED ||
                   this->getCurrentState() == TransferState::ERROR;
        });
