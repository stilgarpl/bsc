...
#include "date.h"
namespace bsc {
    bool bsc::GenericFileReader::isAlgorithmSuitableForFileType(const bsc::FileType& type) { return true; }

    FileMetaData bsc::GenericFileReader::readMetaData(fs::path path) {
        using namespace std::chrono;
        using namespace date;
        FileMetaData result;
        if (fs::exists(path)) {
            result[prefix + ".size"] = std::to_string(fs::file_size(path));
            ...
            result["date.month"] = result["file.date.month"] = std::to_string(static_cast<unsigned int>(ymd.month()));
            result["date.day"] = result["file.date.day"] = std::to_string(static_cast<unsigned int>(ymd.day()));
